...
def solution(sequence, k):
    ...
    # 부분 수열의 합 초기화
    current_sum = 0
    # 현재까지 찾은 가장 짧은 수열의 길이 (무한대로 초기화)
    min_len = float('inf')
    
    # 끝 포인터를 수열의 끝까지 이동시키는 반복문
    for end in range(len(sequence)):
        # 현재까지의 합 (부분 수열의 합)
        current_sum += sequence[end]    
        # 현재 부분 수열의 합이 k보다 크다면
        if current_sum > k:
            # 부분 수열의 합이 k보다 작아질때까지
            # 시작 포인터가 가리키는 값을 부분 수열의 합에서 빼고,
            # 시작 포인터를 한칸 오른쪽으로 이동
            while current_sum > k:
        ...
        if current_sum == k:
            ...
            # 만약 현재 부분 수열의 길이가 
            # 현재까지 찾은 가장 짧은 수열의 길이보다 작다면
            # 가장 짧은 수열의 길이를 현재 길이로 갱신하고
            # answer를 현재 포인터 (start, end)로 갱신

            # 이렇게 조건에 맞는 부분 수열을 찾았으니까
            # 시작 포인터가 가리키는 값을 부분 수열의 합에서 빼고,
            # 시작 포인터를 오른쪽으로 한 칸 이동시키기
